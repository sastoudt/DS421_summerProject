...
toCompare=as.data.frame(cbind.data.frame(allData$chl,allData$Station,yHat))
names(toCompare)=c("trueVal","station","noD")

toCompare$residSq=(toCompare$trueVal-toCompare$noD)^2
by_station=group_by(toCompare,station)
predByStation=summarise(by_station,count=n(),sumSqEr=sum(residSq))
...
g1<-ggplot(forMap,aes(x = Longitude, y = Latitude,colour=toPlot,cex=2))+geom_point()+
  ggtitle("no D matrix RMSE by Station")+scale_size(guide=F)+annotate("text", x=forMap$Longitude,y=forMap$Latitude, label = shreve.order,col="white",cex=5)

## a little worse for rightmost, and cluster of three close to right than for rmses from other models
...
test=getSummaryRMSE(allData,"noD")
test
