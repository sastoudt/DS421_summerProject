...
which(flattenD2[,3]!=0) ## replace with zero
#=flattenD2[which(flattenD2[,3]==0),]

pickFrom=unique(flattenD2[,3])[which(unique(flattenD2[,3])!=0)]

sample(1:length(pickFrom),1)

flattenD2flip=flattenD2
flattenD2flip[which(flattenD2[,3]!=0),3]=rep(0,length(which(flattenD2[,3]!=0)))
flattenD2flip[which(flattenD2[,3]==0),3]=pickFrom[sample(1:length(pickFrom),length(which(flattenD2[,3]==0)),replace=T)]

flattenD2flipM=matrix(0,nrow=13,ncol=13)
for(i in 1:nrow(flattenD2flip)){
  row=flattenD2flip[i,]
  flattenD2flipM[row[1],row[2]]=row[3]
}

flattenD4<-c()
for(i in 1:13){
  for(j in 1:13){
    flattenD4=rbind(flattenD4,c(i,j,D4[i,j]))
  }
}

#which(flattenD4[,3]==0)

#=flattenD4[which(flattenD4[,3]==0),]

pickFrom=unique(flattenD4[,3])[which(unique(flattenD4[,3])!=0)]

flattenD4flip=flattenD4
flattenD4flip[which(flattenD4[,3]!=0),3]=rep(0,length(which(flattenD4[,3]!=0)))
flattenD4flip[which(flattenD4[,3]==0),3]=pickFrom[sample(1:length(pickFrom),length(which(flattenD4[,3]==0)),replace=T)]

flattenD4flip

flattenD4flipM=matrix(0,nrow=13,ncol=13)
for(i in 1:nrow(flattenD4flip)){
  row=flattenD4flip[i,]
  flattenD4flipM[row[1],row[2]]=row[3]
}
...
#Dunif=matrix(runif(13^2,0,1),nrow=13,ncol=13)
#DnormAbs=matrix(abs(rnorm(13^2,0,10)),nrow=13,ncol=13)
#DnormAbsExtreme=matrix(abs(rnorm(13^2,0,50)),nrow=13,ncol=13)
...
P=bdiag.spam(0,lambdaD*t(D2flipM)%*%D2flipM,lambdaT*t(temporal)%*%temporal, lambdaS*t(seasonal)%*%seasonal)
P2=bdiag.spam(0,lambdaD*t(D4flipM)%*%D2flipM,lambdaT*t(temporal)%*%temporal, lambdaS*t(seasonal)%*%seasonal)
...
## 0.0007726829 0.0011072775 0.0017167556 0.0017319700 0.0005659826 0.0011345871 
