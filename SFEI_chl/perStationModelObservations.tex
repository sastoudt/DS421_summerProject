...
\begin{itemize}
\item C10: fitted values look close to the other models above, doesn't get quite the height needed at some peaks, chl component fairly flat except for the beginning of the series 
\item C3: fitted values look close to the other models above, chl component, has variation but on a small order of magnitude
\item D10: fitted values look good, especially after 1990, variation in chl component, larger magnitude than that of doy and date.dec
\item D12: fitted values look pretty good, doesn't get higher magnitudes at the beginning of the series, variation in chl of higher magnitude than date.dec and doy
\item D19: fitted values look pretty good, lots of variation in the chl component
\item D22: fitted values look great, variation in chl is pretty large, but doy and date.dec have larger magnitudes
\item D26: fitted values look good, chl component has more variability until about 1995
\item D28A: fitted values look good, more variability in the chl component until the mid 1990s
\item D4: fitted values look good, magnitude of variation in chl component is larger in the first half of the series
\item D41: fitted values rough, don't really match up well, but general magnitude looks decent, smaller variation in chl component especially towards the end of the series
\item D6: rough fitted values until the 90s, general shape looks decent, but bad magnitudes early on, chl component order of magnitude like doy
\item D7: fitted values look good, lots of variation in chl component, larger magnitude early on
\item D8:  fitted values look good, a bit rougher towards the beginning of the series,lots of variation in chl component, larger magnitude early on
\item MD10: fitted values look good, don't quite get the large magnitudes, fairly consistent magnitude of chl component
\item P8: fitted values look great, fairly small order of magnitude for chl component
\end{itemize}

\subsection{Water Flow Models}

\begin{itemize}
\item C10: Often peaks are too high on fitted values, flow component flat
\item C3: fitted values completely off until end of the 90s, flow component has some variation until the mid 90s where it goes flat, date.dec gets weird after the mid 90s too
\item D10: fitted values pretty rough until the end of the 80s, then match pretty well, total flow component is flat
\item D12: fitted values look pretty good, a bit rougher at the beginning of the series, flat component for flow
\item D19: fitted values are not good, total component constant and very large, intercept large negative
\item D22: fitted values look good, don't get the higher magnitudes at the beginning of the series, total flow component is flat
\item D26: fitted values until 1995 are huge (can't see on shiny because of scale),total component further up, variation until the end of the 90s then flat
\item D28A: fitted values don't match up until the end of the 90s, some peaks in the tot component, early 80s, mid 90s
\item D4: fitted values pretty rough until the 90s, don't get enough height, a few blips in the tot component, but otherwise pretty flat
\item D41: fitted values rough, don't really match up well, general magnitude a bit smaller than for the chl model, flat component for flow
\item D6: fitted values look bad until the end of the 90s, lots of variation in tot until 1995, then flat
\item D7: fitted values look a bit off until the end of the 80s, then look decent, flat total flow component except for a few blips
\item D8: fitted values look good, a bit rougher towards the beginning of the series, flat total flow component
\item MD10: fitted values are a mess until the end of the 90s, flat total flow component except for a few blips
\item P8: fitted values off until the end of the 90s, flow component flat except for a few blips
\end{itemize}
